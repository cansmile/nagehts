...
if($mc < 3) {
	...
	if($fn == "inthebeginning" || $fn == "") {
		...
		// 작업 단원($rm) 이하일 때
		if(($i-2) < $rm) {
			if(file_exists($root.$t1.".php")) {
					echo("      <div class=\"{$col} text-center\">\n       <a href=\"{$t1}\" class=\"text-dark\">\n<img class=\"rounded-circle\" alt=\"{$t2}\" style=\"width: 140px; height: 140px;\" src=\"{$root}<?php echo($root); ?>images/{$t4}.png\" data-holder-rendered=\"true\">\n        <h3>{$t2}</h3>\n        <p>\n{$t3}<br></p>\n       </a>\n      </div>\n");
			} else if(!file_exists($root.$t1.".php")) {
	...
	// Reihe n - n
		...
		for($i = 0; $i < $mc; $i++) {
			...
			if(file_exists($root.$t1.".php")) {
					echo("      <div class=\"{$col} text-center\">\n       <a href=\"{$t1}\" class=\"text-dark\">\n<img class=\"rounded-circle\" alt=\"".substr($t2,2)."\" style=\"width: 140px; height: 140px;\" src=\"{$root}<?php echo($root); ?>images/{$t4}.png\" data-holder-rendered=\"true\">\n        <h3>".substr($t2,2)."</h3>\n        <p>{$t3}</p>\n       </a>\n      </div>\n");
			} else if(!file_exists($root.$t1.".php")) {

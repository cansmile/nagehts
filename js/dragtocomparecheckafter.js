/* 드래그하여 비교/연결하기 (정답 후 확인 버전)
 *
 * taptocomparecheckafter.js를 대체하는 드래그 앤 드롭 인터랙션
...
(function () {
  ...
  function checkPairAndPlaySound(itemEl, targetGroup) {
    var ansGroup = getAnswerGroup(itemEl);

    // 기본 그룹 매칭 확인
    if (ansGroup !== targetGroup) {
      if (typeof x !== 'undefined' && x.play) x.play();
      return; // 소리만 재생, 넣기는 항상 수행
    }

    // cp 배열이 있으면 짝 검증
    if (typeof cp !== 'undefined' && Array.isArray(cp)) {
      var th = itemEl.closest('th[scope=row]');
      if (th) {
        var thNum = parseInt(th.id.substr(3), 10);
        var ths = $('th[scope=row]').length;
        var half = ths / 2;
        var oppNum = thNum <= half ? thNum + half : thNum - half;

        var bt = $('#th-' + oppNum).find('button');
        if (bt.length) {
          var bti = bt.attr('id');
          var cpi = cp[parseInt(bti, 10) - 1];
          ...
          if (cpi) {
            ...
            // cpi가 없으면 반대쪽이 #itms에 있는지 확인
            var itemId = parseInt(itemEl.id, 10);
            var expectedPair = cp[itemId - 1];
            if ($('#itms').find('button#' + expectedPair).length) {
              if (typeof o !== 'undefined' && o.play) o.play();
            } else {
              if (typeof x !== 'undefined' && x.play) x.play();
            }
  ...
  function handleDrop(target) {
    ...
    // 소리 재생 (정답/오답 판별)
    checkPairAndPlaySound(dragging, targetGroup);
